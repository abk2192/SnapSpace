...
export class MainPanelView {
    ...
    bindEvents() {
        ...
        document.body.addEventListener("click", (e) => {
            ...
            if (unlinkBtn) {
                e.stopPropagation(); const [sourceId, targetId] = unlinkBtn.dataset.unlink.split('|');
                this.vm.unlinkItem(sourceId, targetId); return;
            }

            const navTo = e.target.closest("[data-nav-to]");
            if (navTo) {
                const targetId = navTo.dataset.navTo;
                const targetItem = this.vm._items.find(i => i.id === targetId);
                if (targetItem) {
                    const tabTag = (targetItem._tags || []).find(t => t.startsWith('tabid:'));
                    if (tabTag) {
                        this.vm.setActiveTab(tabTag.split(':')[1], 'maximized');
                        setTimeout(() => { const targetCard = document.querySelector(`details.scenario[data-sid="${targetId}"]`); if (targetCard) { targetCard.open = true; targetCard.scrollIntoView({ behavior: 'smooth', block: 'center' }); targetCard.style.boxShadow = "0 0 0 3px var(--primary)"; setTimeout(() => targetCard.style.boxShadow = "var(--shadow)", 2000); } }, 50);
                    }
                }
                return;
            }
    ...
    renderScenarioCard(sc, idx, isDashboard = false, isTempSearch = false) {
        ...
        const links = [...new Set([...(sc.linkedTo || []), ...(sc.linkedFrom || [])])];
        const linkBadges = links.map(id => {
            const linkedItem = this.vm._items.find(i => i.id === id);
            const name = linkedItem ? linkedItem.name : "Linked Note";
            return `<span class="tag dash-badge-link" style="background: transparent; border: 1px solid var(--primary); color: var(--primary); cursor: pointer; display: inline-flex; align-items: center; padding-right: 4px;" data-nav-to="${id}"><span class="material-symbols-outlined" style="font-size:12px; margin-right:4px;">link</span>${escapeHtml(name)}<span class="material-symbols-outlined" style="font-size:14px; margin-left:6px; opacity:0.6;" data-unlink="${sc.id}|${id}" title="Unlink">close</span></span>`;
        }).join("");
        const linksSection = links.length > 0 ? `<div class="label" style="margin-top:12px; margin-bottom: 6px;"><span class="material-symbols-outlined" style="font-size: 14px;">hub</span> Linked Notes</div><div style="display:flex; flex-wrap:wrap; gap:6px;">${linkBadges}</div>` : '';
